...
const TL: Segment[] = [
  { kind: 'beat', move: 30, chips: 2, frames: 35, cold: true, caption: 'A market picks every move.' },
  run(0, 9, 24, 12),
  { kind: 'beat', move: 10, chips: 3, frames: 45 },
  run(10, 39, 24, 37),
  { kind: 'beat', move: 40, chips: 0, frames: 15 },
...
describe('the frame description', () => {
  ...
  it('a beat holds the entry before the move through the chips and the lock, then glides in over its last 12 frames', () => {
    const b = start(2); // move 10, 3 chips: 30 frames of chips, 3 of lock, 12 of move
    expect(frameAt(TL, entries, b).position).toBe(9);
    expect(frameAt(TL, entries, b + 29).position).toBe(9);
    expect(frameAt(TL, entries, b + 32).position).toBe(9);
    expect(frameAt(TL, entries, b + 33).position).toBeGreaterThan(9);
    expect(frameAt(TL, entries, b + 38).position).toBeCloseTo(9.5, 9); // halfway through the 12-frame glide
    expect(frameAt(TL, entries, b + 44).position).toBe(10);
  });

  it('chip k plays during frames 10k to 10k + 9, then the lock, then the move', () => {
    const b = start(2);
    expect(frameAt(TL, entries, b).beat).toMatchObject({ move: 10, phase: 'chips', chip: 0, progress: 0 });
    expect(frameAt(TL, entries, b + 9).beat).toMatchObject({ phase: 'chips', chip: 0 });
    expect(frameAt(TL, entries, b + 10).beat).toMatchObject({ phase: 'chips', chip: 1, progress: 0 });
    expect(frameAt(TL, entries, b + 15).beat).toMatchObject({ phase: 'chips', chip: 1, progress: 0.5 });
    expect(frameAt(TL, entries, b + 20).beat).toMatchObject({ phase: 'chips', chip: 2, progress: 0 });
    expect(frameAt(TL, entries, b + 30).beat).toMatchObject({ phase: 'lock' });
    expect(frameAt(TL, entries, b + 33).beat).toMatchObject({ phase: 'move' });
  ...
  it('the push-in: 1 outside beats, easing to 1.08 during one and back to 1 at its end', () => {
    ...
    expect(frameAt(TL, entries, b + 44).zoom).toBeLessThan(1.02);
  ...
  it('a beat with a caption shows it for the whole beat, and no other frame has one', () => {
    ...
    expect(frameAt(TL, entries, 34).caption).toBe('A market picks every move.');
    expect(frameAt(TL, entries, 35).caption).toBeNull();
  ...
  it('a record card follows the crash that ends a record attempt for 60 frames', () => {
    ...
    const b = start(2);
    const during = frameAt(TL, entries, b + 44).lowerThird;
    expect(during).toMatchObject({ text: 'RECORD 6 · attempt 1' });
    expect(frameAt(TL, entries, b + 44 + 59).lowerThird).not.toBeNull();
    expect(frameAt(TL, entries, b + 44 + 60).lowerThird).toBeNull();
  ...
  it('a cold open on a record crash brings no card; the card comes when the story reaches the crash', () => {
    const cold: Segment[] = [
      { kind: 'beat', move: 10, chips: 1, frames: 25, cold: true, caption: 'A market picks every move.' },
      run(0, 9, 24, 12),
      { kind: 'beat', move: 10, chips: 1, frames: 25 },
      { kind: 'credits', frames: 540 },
    ];
    for (let f = 0; f < 25; f++) expect(frameAt(cold, entries, f).lowerThird, String(f)).toBeNull();
    // the story's beat on move 10 starts at frame 37 and arrives on its last frame, 61
    expect(frameAt(cold, entries, 60).lowerThird).toBeNull();
    expect(frameAt(cold, entries, 61).lowerThird).toMatchObject({ text: 'RECORD 6 · attempt 1' });
  });
