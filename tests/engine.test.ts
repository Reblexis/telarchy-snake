...
describe('the game (docs/snake.md, "The game")', () => {
  it('starts at the centre with length 3, heading right, one food on a free cell', () => {
    const g = newGame(rng([0]));
    expect(GRID).toBe(20);
    expect(g.snake.length).toBe(3);
    expect(g.heading).toBe('right');
    expect(g.snake[0]).toEqual({ x: 10, y: 10 });
    expect(g.snake).toEqual([{ x: 10, y: 10 }, { x: 9, y: 10 }, { x: 8, y: 10 }]);
  ...
  it('eating food grows the snake by one and spawns new food on a free cell', () => {
    ...
    expect(g2.snake.length).toBe(4);
    expect(g2.length).toBe(4);
    expect(g2.food).not.toEqual({ x: 11, y: 10 });
    expect(g2.snake.some(s => s.x === g2.food.x && s.y === g2.food.y)).toBe(false);
  });

  it('hitting a wall is a death: respawn at once in the starting state, deaths counted', () => {
    let g = newGame(rng([0]));
    g = { ...g, snake: [{ x: 19, y: 10 }, { x: 18, y: 10 }, { x: 17, y: 10 }], heading: 'right' };
    const g2 = step(g, 'right');
    expect(g2.deaths).toBe(1);
    expect(g2.snake).toEqual([{ x: 10, y: 10 }, { x: 9, y: 10 }, { x: 8, y: 10 }]);
  ...
  it('death resets length to 3 but keeps deaths and steps', () => {
    let g = newGame(rng([0]));
