...
describe('the game (docs/snake.md, "The game")', () => {
  it('starts at the centre with length 2, heading right, one food on a free cell', () => {
    ...
    expect(g.snake).toEqual([{ x: 2, y: 2 }, { x: 1, y: 2 }]);
  ...
  it('food never spawns on the snake even when the rng keeps pointing at it', () => {
    let g = newGame(rng([78, 78, 78, 0]), 12, 1);
    g = { ...g, food: { x: 7, y: 6 } };
    const g2 = step(g, 'right');
    expect(g2.snake.some(s => s.x === g2.food.x && s.y === g2.food.y)).toBe(false);
  });
  ...
  it('a later game is two cells larger, numbered, starts at the centre at length 2', () => {
    const g = newGame(rng([0]), 6, 2);
    expect(g.size).toBe(6);
    expect(g.gameNumber).toBe(2);
    expect(g.snake[0]).toEqual({ x: 3, y: 3 });
    expect(g.length).toBe(2);
    // the far wall is at 5 on a 6-grid
    let h = { ...g, snake: [{ x: 5, y: 3 }, { x: 4, y: 3 }], heading: 'right' as Direction };
    expect(step(h, 'right').deaths).toBe(1);
    h = { ...g, snake: [{ x: 4, y: 3 }, { x: 3, y: 3 }], heading: 'right' as Direction };
    expect(step(h, 'right').deaths).toBe(0);
  });

  it('death on a larger grid respawns on the same grid, not the first one', () => {
    let g = newGame(rng([0]), 6, 2);
    g = { ...g, snake: [{ x: 5, y: 3 }, { x: 4, y: 3 }], heading: 'right' };
    const g2 = step(g, 'right');
    expect(g2.size).toBe(6);
    expect(g2.gameNumber).toBe(2);
    expect(g2.snake[0]).toEqual({ x: 3, y: 3 });
  });

  it('filling the grid completes the game: length 16, complete, and further steps change nothing', () => {
    // A snake of 15 cells in a boustrophedon with the last free cell at the end of the path.
    const cells: { x: number; y: number }[] = [];
    for (let y = 0; y < GRID; y++) for (let i = 0; i < GRID; i++) cells.push({ x: y % 2 === 0 ? i : GRID - 1 - i, y });
    const body = cells.slice(0, GRID * GRID - 1).reverse();
    const free = cells[GRID * GRID - 1];
    let g = newGame(rng([0]));
    g = { ...g, snake: body, heading: 'right', length: GRID * GRID - 1, food: free };
    const dir = free.x > body[0].x ? 'right' : free.x < body[0].x ? 'left' : free.y > body[0].y ? 'down' : 'up';
    const g2 = step(g, dir as Direction);
    expect(g2.length).toBe(GRID * GRID);
  ...
  it('attemptStep counts the moves of the current attempt: up from 0 at a start, back to 0 on a death', () => {
    let g = newGame(() => 0, 12, 1); // the counter, not the grid: room to walk
